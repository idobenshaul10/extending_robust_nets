...
def show_histogram(diffs):	
	...
	plt.title(
		"Minimal seperation histogram over Test Dataset"
	)
	# plt.xticks(np.arange(2, np.max(list(Counter(diffs).keys()))))
	plt.xlabel('Minimal Separation')
...
for i in tqdm(range(len(cohort))):
	...
	min_diff = findMinSeparation(sample)	
...
# noise_rel = torch.tensor(np.arange(0.0, 0.6, 0.05))
...
for (idx, method) in methods.iterrows():
	if idx not in methods_no_calc:
		...
		s_len = X_0.shape[0]
		results.loc[idx].X_err = torch.zeros(len(noise_rel), len(minimal_differences), s_len)

		for current_minimal_separator in minimal_differences:

			cur_samples = np.where(min_diffs == current_minimal_separator)[0]
			for s in tqdm(range(s_len)):
				...
				for idx_noise in range(len(noise_rel)):                
					...
					try:
						results.loc[idx].X_err[idx_noise, (current_minimal_separator-2), s] = err_measure(
							cur_X, X_0_s
						).mean()
					except:
...
if do_plot:
	...
	ax.set_yticks(minimal_differences)
	# ax.set_zticks(np.arange(0, 1, step=0.05))
	
	ax.set_xlabel('noise percent')
	ax.set_ylabel('minimal difference')
	ax.set_zlabel("rel.\\ $\\ell_2$ err. percent")
	plt.title(
		"reconstruction rel.\\ $\\ell_2$ for $m=100$ coefficients, w.r.t Minimal Separation"
	)
	# import pdb; pdb.set_trace()
