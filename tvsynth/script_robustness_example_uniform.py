...
from operators import noise_uniform
...
save_results = os.path.join(save_path, "example_S6_uniform.pkl")
...
if do_plot:
        ...
        for idx_noise in range(len(noise_rel_show)):
            ...
            if save_plot:
                fig.savefig(
                    os.path.join(
                        save_path,
                        "fig_example_S{}_uniform_".format(sample)
    ...
    if save_plot:
        fig.savefig(
            os.path.join(
                save_path, "fig_example_S{}_uniform_curve.pdf".format(sample)
            ),
            bbox_inches="tight",
        )
